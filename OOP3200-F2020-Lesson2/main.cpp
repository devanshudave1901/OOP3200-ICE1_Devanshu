...
// costructor for cartesian
CartesianPoint::CartesianPoint(int x, int y) {
	// set a point for x and y -> SetPoint(x,y)
	SetPoint(x, y);

}
// Setter...seeting the SetPoint().
   void CartesianPoint::SetPoint(int x, int y) {
	SetX(x);
	SetY(y);
}

// setter(mutator) for SetX
...
//  setter(mutator) for SetY
void CartesianPoint::SetY(int y)
{
	myY = y;
}

// Getter method for Getting X
